...
class Replica(Node):
    ...
    def commit_prev_operation(self):
        ...
        if self.commit_num < self.last_commit_num and has_last_commited:
            ...
            for op in range(my_commit, has_last_commited):
                ...
                self.commit_num += 1
                if client not in self.client_table:
                    self.client_table[client] = [req, None, None]
            ...
            self.commit_num += 1
            if m[1] not in self.client_table:
                self.client_table[m[1]] = [m[0], None, None]
    ...
    def run_query(self, message):
        (order, m) = message
        logger.info(f"Replica:{self.node_id}:{time.monotonic_ns()}: Received query: {order} {m}")
...
class Client:
    def __init__(self, replicas: list[Replica], tout, addr, scenario = "best"):
        # Socket
    ...
    def start(self):
        ...
        with open(f'queries/{self.scenario}/vsr/query_1.txt', 'r') as f:
            for i, line in enumerate(f.readlines()):
                ...
                if query == "SET":
                    data = ("REQUEST", (payload, self.c_id, self.req_num))
                    self.last_operation = data
                    self.send_to(self.primary, data)

                if query == "READ":
                    data = ("READ", (payload, self.c_id, self.req_num))
                    self.last_operation = data
                    self.send_to(self.primary, data)
    ...
    def send_to(self, addr, data):
        threading.Thread(target=addr.run_query, args=(data,)).start()
